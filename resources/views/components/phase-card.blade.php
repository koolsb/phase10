...
@php
    ...
    $padding    = match($size) { 'xl' => 'px-10 pt-10 pb-8', 'lg' => 'px-8 pt-8 pb-7', default => 'px-7 pt-7 pb-6' };
    $titleSize  = match($size) { 'xl' => 'text-5xl', 'lg' => 'text-4xl', default => 'text-3xl' };
    $subSize    = match($size) { 'xl' => 'text-sm', 'lg' => 'text-sm', default => 'text-xs' };
    $spacing    = match($size) { 'xl' => 'space-y-5', 'lg' => 'space-y-3.5', default => 'space-y-2.5' };
    $itemSize   = match($size) { 'xl' => 'text-[22px]', 'lg' => 'text-[18px]', default => 'text-[15px]' };
    $numW       = match($size) { 'xl' => 'w-9', 'lg' => 'w-7', default => 'w-6' };
    $badgeSize  = match($size) { 'xl' => 'text-[13px]', 'lg' => 'text-[11px]', default => 'text-[10px]' };
    $stripe     = match($size) { 'xl' => 'h-4', 'lg' => 'h-3.5', default => 'h-2.5' };
@endphp
...
<div {{ $attributes->class("phase10-card-bg relative mx-auto w-full {$maxW} overflow-hidden rounded-2xl text-white shadow-xl ring-1 ring-black/10") }}>
    <div class="{{ $padding }}">
        ...
        <ol class="mt-6 {{ $spacing }}">
            @foreach ($phases as $i => $phase)
                <li class="flex items-baseline gap-3 {{ $itemSize }} leading-snug">
